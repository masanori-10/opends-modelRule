...
*/	
	...
	private float getPTire(float velocity, float vehicleMass, float gravityConstant) 
	{
		...
		boolean isSnow = EffectCenter.getSnowingPercentage() > 20;
		boolean isRain = EffectCenter.getRainingPercentage() > 20;
		if (isSnow) {
			if (isRain) {
				rollingResistanceCoefficient *= 0.07 / 0.7;
			} else {
				rollingResistanceCoefficient *= 0.15 / 0.7;
			}
		} else if (isRain) {
			rollingResistanceCoefficient *= 0.5 / 0.7;
		}
